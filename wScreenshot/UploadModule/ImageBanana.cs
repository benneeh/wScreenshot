...
{
    public class ImageBanana
    {
        //public string Upload(Bitmap bmp, ImageFormat imgf, string fileName, string user, string pass)
        //{
        ...
        //        HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create("http://www.imagebanana.com/");
        ...
        //        contents.AppendLine(string.Format("Content-Disposition: form-data; name=\"upload[]\"; filename=\"{0}\"", fileName));
        ...
        //                    Match m = Regex.Match(ret, "\\[url=(.*?)\\]", RegexOptions.IgnoreCase);
        //                    if (m.Success)
        //                    {
        ...
        //            if (ret != "Error")
        //            {
        //                using (WebResponse response = HttpWebRequest.Create(ret).GetResponse())
        //                {
        //                    using (StreamReader reader = new StreamReader(response.GetResponseStream()))
        //                    {
        //                        ret = reader.ReadToEnd();
        //                        Match m = Regex.Match(ret, "id=\"image\" src=\"(.*?)\" alt=\".*?\" />", RegexOptions.IgnoreCase);
        //                        if (m.Success)
        //                        {
        //                            ret = m.Groups[1].Value;
        //                        }
