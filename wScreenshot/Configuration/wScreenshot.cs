...
namespace wScreenshot.Configuration
{
    ...
    public class wScreenshot : INotifyPropertyChanged, INotifyPropertyChanging
    {
        ...
        {
            get { return Environment.SpecialFolder.ApplicationData; }
        }
        ...
        public string FullPath
        {
            get
            {
                return Path.Combine(Environment.GetFolderPath(Location), Assembly.GetExecutingAssembly().GetName().Name,
                    ConfigVersion, string.Format(@"{0}.config", ConfigName));
            }
        }
        ...
        public void Save()
        {
            ...
            using (
                FileStream fs = fi.Open(FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite | FileShare.Delete))
            {
